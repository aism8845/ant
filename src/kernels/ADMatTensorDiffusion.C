...
registerMooseObject("AntApp", ADMatTensorDiffusion);
...
ADMatTensorDiffusion::validParams()
{
  ...
  params.addRequiredParam<MaterialPropertyName>("diffusivity",
                                                "Tensor diffusivity material property name.");
...
ADMatTensorDiffusion::ADMatTensorDiffusion(const InputParameters & parameters)
  : ADKernelGrad(parameters),
    _diffusivity(
        getADMaterialProperty<RealTensorValue>(getParam<MaterialPropertyName>("diffusivity")))
{
}
