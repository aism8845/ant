...
registerMooseObject("AntApp", ADMatReactionSigned);
...
ADMatReactionSigned::validParams()
{
  ...
  params.addRequiredParam<MaterialPropertyName>("reaction_rate",
                                                "Reaction rate material property name.");
  params.addClassDescription(
      "AD reaction kernel with residual contribution +reaction_rate * u.");
  return params;
}
...
ADMatReactionSigned::ADMatReactionSigned(const InputParameters & parameters)
  : ADKernelValue(parameters),
    _reaction_rate(
        getADMaterialProperty<Real>(getParam<MaterialPropertyName>("reaction_rate")))
{
}
